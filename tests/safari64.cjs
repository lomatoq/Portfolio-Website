// Original GPU material, cached glow, viewport coverage and alpha regressions. Desktop WebKit is not an iPhone FPS test.
...
(async()=>{for(const [name,type] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  assert.equal(material.renderer.glassComposition,'world-webgl');assert(material.renderer.visibleGlass>0);
  assert.equal(material.background,'none');assert(material.grain[0]<=393&&material.grain[1]<=852);
  ...
  const viewport=await p.evaluate(()=>{const e=document.querySelector('.boot-veil');const hidden=e.hidden;e.hidden=false;const r=e.getBoundingClientRect();e.hidden=hidden;return {top:r.top,bottom:r.bottom,height:innerHeight};});
  assert(viewport.top<=0&&viewport.bottom>=viewport.height,'veil does not cover viewport');
  await p.waitForFunction(()=>document.querySelectorAll('.glow-cached').length===10,null,{timeout:30000});
